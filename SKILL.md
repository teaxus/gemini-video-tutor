...
2. **判断输入类型**：
   - **本地文件** → 直接分析。
   - **在线 URL**（http/https、youtube/bilibili/抖音等）→ 本 skill 不下载。先确认 video-downloader 已安装（setup.py 会报告；未装可 `python3 ./scripts/setup.py --install-downloader` 自动克隆），用它下载后再分析。
   - **⚠️ 下载位置**：通过 video-downloader 的第三个位置参数显式指定，取值规则：
     1. config `video.download_dir` 非空 → 用它（用户的固定视频库目录）；
     2. 否则（默认）→ **本次分析的产物目录**（即 `-o` 所在目录）。
     ```bash
     python3 <video-downloader>/scripts/video_downloader.py "<URL>" 1080p "<下载目录>"
     ```
     不要省略该参数放任缺省——视频与分析产物同目录才方便用户整体归档/删除，且分析文档元数据记录视频路径，`--resume` 与 `ask.py` 会话都依赖它长期有效。默认产出形态：`分析目录/{视频.mp4, 分析.md, 分析_frames/}`。
3. **选模式**（关键分流）：
...
**接口协议 `api_style`**（关键兼容点）：`auto`（默认）下，官方 `googleapis.com` 走 Gemini 原生协议（含 File API），**其它端点一律按 OpenAI 兼容中转处理**（OpenRouter/one-api/new-api 的事实标准，视频以 base64 `video_url` 内联发送）。中转是 Gemini 原生透传时需显式设 `api_style: "gemini"`。
...
# 批量并发：目录或列表文件。每个视频一个子目录（重名自动加序号），
# 已完成的自动跳过（断了重跑即续传，--force 强制全重跑），结束生成 _batch_report.md
